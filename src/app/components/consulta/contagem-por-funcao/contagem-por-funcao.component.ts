...
@Component({
  selector: 'app-contagem-por-funcao',
  templateUrl: './contagem-por-funcao.component.html',
  styleUrls: ['./contagem-por-funcao.component.css'],
...
export class ContagemPorFuncaoComponent implements OnInit {

  contagem: { funcao: string, quantidade: number }[] = [];
  ...
  contagemPorFuncao() {
    ...
    this.apiService.contagemPorFuncao(dataFormatada)
      ...
      .subscribe(data => {
        this.contagem = Object
          .entries(data)
          .map(([funcao, quantidade]) => ({ funcao, quantidade: quantidade as number }));
      });
